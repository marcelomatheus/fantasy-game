...
Esse game evolui a vertical slice anterior sem substituir o núcleo que tornou o combate responsivo. A mesma simulação de `Fighter` + `CombatSystem` + `RoundManager` continua responsável por movimento, commitment windows, hitboxes/hurtboxes, hit stop, hit stun, knockback, block, combos, câmera e rounds; a v3 profissionaliza as camadas ao redor dela.
