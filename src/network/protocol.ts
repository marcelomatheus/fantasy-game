...
export interface RoomPlayer { sessionId:string; name:string; fighterId:string; skinId:string; ready:boolean; host:boolean; connected:boolean; ping:number; status:PlayerRoomStatus; }
...
export interface MatchStartPayload { matchId:string; roomCode:string; p1Session:string; p2Session:string; fighterIds:[string,string]; skinIds:[string,string]; settings:MatchSettings; tournament:boolean; }

export type ClientMessage=
  | {type:'hello';sessionId:string;name:string;resumeToken?:string}
