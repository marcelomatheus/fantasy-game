...
import type { ClientMessage, MatchStartPayload, RoomMode, RoomState, ServerMessage } from './protocol.js';

type EventMap={
  status:string; room:RoomState; match:MatchStartPayload; input:{matchId:string;frame:number;bits:number;fromSession:string}; matchEnded:{matchId:string;winnerSession:string}; tournamentComplete:{championSession:string}; notice:string; error:string;
};
...
export class NetworkClient {
  ...
  connect(name=this.playerName):void{
    ...
    try{const ws=new WebSocket(url);this.socket=ws;
      ws.onopen=()=>{this.reconnectAttempts=0;this.emit('status','Online');this.send({type:'hello',sessionId:this.sessionId,name:this.playerName,resumeToken:this.resumeToken||undefined});this.startPing();};
  ...
  sendInput(matchId:string,frame:number,bits:number):void{this.send({type:'input',matchId,frame,bits});}
  ...
  private handleMessage(raw:string):void{let msg:ServerMessage;try{msg=JSON.parse(raw) as ServerMessage;}catch{return;}
    ...
    if(msg.type==='pong'){this.ping=Math.max(0,Math.round(performance.now()-msg.sentAt));this.send({type:'pingReport',ping:this.ping});}
  }
  ...
  private startPing():void{this.stopPing();const ping=()=>{if(this.connected)this.send({type:'ping',sentAt:performance.now()});};ping();this.pingTimer=window.setInterval(ping,2000);}
