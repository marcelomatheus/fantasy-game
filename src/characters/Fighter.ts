...
import type { AttackDefinition, AttackKey, Controls, Facing, FighterDefinition, FighterSimulationState, FighterSnapshot, FighterState, MoveKey, PlayerId, Rect, SpecialKey, SpecialMoveDefinition } from '../types/game.js';
...
export class Fighter {
  ...
  simulationState():FighterSimulationState{return{x:this.x,y:this.y,vx:this.vx,vy:this.vy,health:this.health,displayedHealth:this.displayedHealth,state:this.state,facing:this.facing,wins:this.wins,meter:this.meter,hitStunMs:this.hitStunMs,blockStunMs:this.blockStunMs,flashMs:this.flashMs,grounded:this.grounded,attackSerial:this.attackSerial,strikeVariant:this.strikeVariant,previousAttack:this.previousAttack,commandClock:this.commandClock,directions:this.directions.map(v=>({...v})),finisherRequest:this.finisherRequest,attack:this.attack?{key:this.attack.definition.key,elapsed:this.attack.elapsed,hitTargets:[...this.attack.hitTargets],serial:this.attack.serial}:null,buffered:this.buffered?{...this.buffered}:null};}
  restoreSimulationState(s:FighterSimulationState):void{this.x=s.x;this.y=s.y;this.vx=s.vx;this.vy=s.vy;this.health=s.health;this.displayedHealth=s.displayedHealth;this.state=s.state;this.facing=s.facing;this.wins=s.wins;this.meter=s.meter;this.hitStunMs=s.hitStunMs;this.blockStunMs=s.blockStunMs;this.flashMs=s.flashMs;this.grounded=s.grounded;this.attackSerial=s.attackSerial;this.strikeVariant=s.strikeVariant;this.previousAttack=s.previousAttack;this.commandClock=s.commandClock;this.directions=s.directions.map(v=>({...v}));this.finisherRequest=s.finisherRequest;this.attack=s.attack?{definition:this.definition(s.attack.key),elapsed:s.attack.elapsed,hitTargets:new Set(s.attack.hitTargets),serial:s.attack.serial}:null;this.buffered=s.buffered?{...s.buffered}:null;}
  quantizeSimulation():void{const q=(v:number)=>Math.round(v*1000)/1000;this.x=q(this.x);this.y=q(this.y);this.vx=q(this.vx);this.vy=q(this.vy);this.health=q(this.health);this.meter=q(this.meter);this.hitStunMs=q(this.hitStunMs);this.blockStunMs=q(this.blockStunMs);this.flashMs=q(this.flashMs);this.commandClock=q(this.commandClock);if(this.attack)this.attack.elapsed=q(this.attack.elapsed);if(this.buffered)this.buffered.remaining=q(this.buffered.remaining);for(const direction of this.directions)direction.at=q(direction.at);}
